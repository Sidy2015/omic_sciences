...
To begin learning aobuto GO set enrichmet analysis and the different methods that can be utiliazed to preform these analysis a good jumping off start point is this Natural Protocols paper:
...
```{r ranked list}
prank = read.delim("Supplementary_Table2_MesenvsImmuno_RNASeq_ranks.rnk",stringsAsFactors = F,check.names = F)
pathways = gmtPathways(gmt.file = "Supplementary_Table3_Human_GOBP_AllPathways_no_GO_iea_July_01_2017_fixed_symbol.gmt")
...
ppos = read.delim(file = "Supplementary_Table8_gsea_report_for_na_pos.txt" ,stringsAsFactors = F,
                  check.names = F)

pneg = read.delim(file = "Supplementary_Table9_gsea_report_for_na_neg.txt" ,stringsAsFactors = F,
                  check.names = F)
...
```{r 2 x 2 table}
     ...
     #  # in order to find N, we need to find how many genes have a non-NA score in         the relevant column of interest
      ...
      ## n = N - m number of “non-marked” elements, i.e. the number of genes that have some              annotation, but are not associated to the selected GO term (ARGININE BIOSYNTHETIC                        PROCESS)
      n = N - m
      ...
      q = x -1
      phyp = phyper(q = x-1,m,n,k,lower.tail = F)#probability that X os greater than overlap
      signif(phyp,3)
...
```{r contingency}
     ...
     ## m is the number of successes in population = geneSetsize ##success in population
     ## n = N - m
