...
<a name="whyr"></a>
The Rmd file for this lesson is available [here](https://github.com/gurinina/omic_sciences/blob/main/02-bioconducter.Rmd)
## Why R Bioconductor?
...
```
Before you tell `BiocManager` to install these packages, check to make sure that the format
is correct. See here I have two errors I need to get rid of, `biomaRt` is surrounded by backslashes
and dplyr has "warn.conflifts". 

```{r,eval=FALSE}


need = c(need, "gwascat","ggupset","ggnewscale")
...
All of your package needs should be met by need and git, i.e., you should have no "needs"
```
One final package that needs installation for the course is the following:
...
you have a potential function conflict with another library that is loaded. This shortened version of the script is in required_packages.R...so that's all you need to run.
