...
found
[here](https://github.com/gurinina/omic_sciences/blob/main/16-GOenrichment.Rmd).
...
if (file.exists("sapGO.rds")) {
  sapGO <- readRDS("sapGO.rds")
} else {
  sapGO <- godata('org.Hs.eg.db', keytype = "SYMBOL", ont="BP", computeIC=TRUE)
  saveRDS(sapGO, "sapGO.rds")
}
...
if (file.exists("scGO.rds")) {
  scGO <- readRDS("scGO.rds")
} else {
  scGO <- godata('org.Sc.sgd.db', keytype = "GENENAME", ont="BP", computeIC=FALSE)
  saveRDS(scGO, "scGO.rds")
}
...
if (file.exists("simp.rds")) {
  simp <- readRDS("simp.rds")
} else {
 simp = clusterProfiler::simplify(hse,semData  = sapGO)
  saveRDS(simp, "simp.rds")
}
...
```{r yanno}
if (file.exists("yanno.rds")) {
  yanno <- readRDS("yanno.rds")
} else {
