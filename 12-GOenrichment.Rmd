...
emapplot(hx, showCategory = 15)
```
Another approach is to collapse the GO terms by semantic similarity.
Because GO is organized hierarchically in a parent-child structure, 
a parent term can overlap with a large proportion with all its child
terms. This means that many of the enriched GO terms are redundant.  
...
```
Another way of measuring redundancy or similarity 
...
calcOverGOcoeff= function(select_fun = overlapcoeff,res, cutoff = 0.6, showCategory = 200, keytype = "GENENAME",ont = "BP",orgDb = "org.Sc.sgd.db") { ## tso satisfy codetools for calling gather
  ...
  jaccard <- function (x, y) {
    ...
    m=m%>% mutate(gotoremove= m%>%select(padj1,padj2)%>% apply(MARGIN = 1, FUN = function(x) which.max(x)))
