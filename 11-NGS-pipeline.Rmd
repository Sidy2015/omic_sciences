...
Study information at the [Sequence Read Archive](http://www.ncbi.nlm.nih.gov/Traces/sra/?study=SRP033351)

[Himes et al paper](http://www.ncbi.nlm.nih.gov/pubmed/24926665)

Example sample table stored in our course repo on github: [airway_sample_table](https://github.com/genomicsclass/labs/blob/master/course5/airway_sample_table.csv)

Details on creating such a sample table from SRA and GEO: [details](http://www.bioconductor.org/packages/release/data/experiment/vignettes/airway/inst/doc/airway.html)

The [European Nucleotide Archive (EMBL-EBI)](http://www.ebi.ac.uk/ena>)

The [Sequence Read Archive (NCBI)](http://www.ncbi.nlm.nih.gov/sra/>)
...
we're going to use a truncated fastq file to save memory, navigate to the directory
...
```{r echo = FALSE,warning=FALSE,  message = FALSE}
library(ShortRead)
fastqFile = fastqcfile
# read fastq file
fq = readFastq(fastqFile[g])
fq
#class: ShortReadQ
