...
```{r Figure 1.(a), fig.cap="Comparison by ethnicity", fig.width=7,fig.height=4}
...
To explore these issues in more detail, the Akey et al. downloaded the raw CEL files from Gene Expression Omnibus (GSE5859) and extracted from the header line the date on which the file was created. Notably, the arrays used to measure expression for the CEU individuals were primarily processed from 2003 to 2004,whereas the arrays used to measure expression for the ASN individuals were all processed in 2005–2006 (see figure1). We tested for differential expression with respect to the year in which the microarrays were processed and found that at least 94% of genes are estimated to be differentially expressed. 

![Figure 2.](figure/batch.png)

Let's look at these arrays:
Figure 1.(b) P-values resulting from tests of differential P-values resulting from tests of differential P expression with respect to year in which the microarrays were processed. 
```{r Figure 1.(b), fig.cap="Comparison by year", fig.width=7,fig.height=4}
...
Figure 1.(c) P-values resulting from tests P-values differential expression between the CEU and ASN samples while controlling for the year in which the sample was processed.
```{r Figure 1.(c), fig.cap="Comparison by batch", fig.width=7,fig.height=4}
...
```{r Figure 1.(d), fig.cap="CEU ethnicity and year", fig.width=7,fig.height=4}
...
![Figure 3.](figure/confounding.png)
...
```{r, fig.cap=Figure 4: "Clustering of the test samples in the bladder cancer study before (left) and after (right) adjustment with the fsva function. After using the fsva function, the normal and biopsy samples cluster together and only one cancer sample is misclustered"}
