...
```
The Rmd for this file can be found [here](https://github.com/gurinina/omic_sciences/blob/main/07-confounding.Rmd). Batch effects have the most devastating effects when they are _confounded_ with outcomes of interest. Here we describe confounding and how it relates to data interpretation.
...
hist(h$data$pvals,freq=F,ylim=c(0,15))


round(length(wq)/nrow(res1)*100,1)
```

It seemed implausible that as many as 78% of genes are differentially expressed between the CEU and ASN samples. For example, based on the complete distribution of P values, the authors estimated that approximately 17% of genes are differentially expressed between individuals of European and African ancestry(2). To rule out the possibility that this difference was related to the larger sample size of Spielman et al.1 and is thus not simply a power issue, we they sampled eight CEU and eight ASN individuals (corresponding to the sample size of Storey et al.(2) 1,000 times, and for each sample estimated the proportion of differentially expressed genes as described above. The average proportion was 43% (standard deviation (s.d.) = 8%, demonstrating that differences in power between our study and Spielman et al.(1) do not fully account for differences in the estimated proportion of differentially expressed genes among human populations. 

To explore these issues in more detail, the authors downloaded the raw CEL files from Gene Expression Omnibus (GSE5859) and extracted from the header line the date on which the file was created. Interestingly, the arrays used to measure expression for the CEU individuals were primarily processed from 2003 to 2004,whereas the arrays used to measure expression for the ASN individuals were all processed in 2005–2006. We tested for differential expression with respect to the year in which the microarrays were processed and found that at least 94% of genes are estimated to be differentially expressed (Fig. 1b).

![](figure/batch.png)

Let's look at these arrays:
(b) P-values resulting from tests of differential P-values resulting from tests of differential P expression with respect to year in which the microarrays were processed. 
```{r volcano_plots, fig.cap="Volcano plots for gene expression data. Comparison by year", fig.width=7,fig.height=4}

##remove control genes
out <- grep("AFFX",rownames(geneExpression))

eth <- sampleInfo$ethnicity
ind2<- which(eth%in%c("CEU","ASN") & year%in% c("02","03","04","05"))
out <- rowout <- grep("AFFX",rownames(geneExpression))

eth <- sampleInfo$ethnicity
ind2<- which(eth%in%c("CEU","ASN") & year%in% c("02","03","04","05") )
res2 <- rowttests(geneExpression[-out,ind2],droplevels(eth[ind2]))
q2 = qvalue(res2$p.value)
...
plot(res2$dm,-log10(res1$p),xlim=XLIM,ylim=YLIM,
     xlab="Effect size",ylab="-log10(p-value)",main="Populations")
hist(h2$data$pvals,freq=F,ylim = c(0,15))

round(length(wq2)/nrow(res2)*100,1)
```
...
We have not yet discussed these methods, but we can model it none the less in the following way, though it's not clear if it is the correct way to model it.

(c) P-values resulting from tests P-values differential expression between the CEU and ASN samples while controlling for the year in which the sample was processed.
```{r volcano_plots, fig.cap="Volcano plots for gene expression data. Comparison by batch", fig.width=7,fig.height=4}
...
```{r volcano_plots, fig.cap="Volcano plots for gene expression data. Comparison by CEU eth and year", fig.width=7,fig.height=4}
...
hist(h4$data$pvals,freq=F,ylim=c(0,10)    )
...
res5 <- rowttests(geneExpression[-out,ind4],droplevels(year[ind4]))
q5 = qvalue(res4$p.value)
wq5 = which(q5$qvalues < 0.05)

i = unique(c(wq3,wq4,wq5))
# of unique genes altered by year for CEU population
round(length(i)/nrow(res5)*100,1)
...
The y-axis in each plot is drawn y-axis in each plot is drawn y to reflect a histogram density, where the total area of all rectangles is 1. Under the null hypothesis of no differential expression, we expect the P-values to be uniformly distributed between 0 and 1, forming P-values to be uniformly distributed between 0 and 1, forming P a histogram with frequencies following the dashed black line. Using well-established methodology(3,4), we estimate the proportion of differentially expressed genes in a–d to be 78%, 94%, 0% and 79%, respectively. The odd shape of the histogram in c is attributable to the almost complete confounding of year of processing and population, illustrating the underlying problem with the study design.
