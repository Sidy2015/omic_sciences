...
The Rmd for this file can be found [here](https://github.com/gurinina/omic_sciences/blob/main/11-RNAseq-limma.Rmd).
Contents
1 Abstract
2 Introduction
3 Set-up
...
```{r}
geneid <- rownames(x)
genes <- AnnotationDbi::select(Mus.musculus, keys=geneid, columns=c("SYMBOL", "TXCHROM"), 
                keytype="ENTREZID")
head(genes)
```
...
```
[Link to Interactive MD](http://bioinf.wehi.edu.au/folders/limmaWorkflow/glimma-plots/MD-Plot.html)
...
## Gene set testing with clusterProfiler and fgsea

We finish off this analysis with (some gene set testing by applying the fgsea method using the [c2 gene signatures gene sets](http://bioinf.wehi.edu.au/software/MSigDB/) from the Broad Institute’s MSigDB c2 collection (Subramanian et al. 2005) that have been adapted for mouse and are available as Rdata objects. We also do an enrichment using the classical GO ontology biological processes with clusterProfiler.
```{r}
library(clusterProfiler)
rank = -log10(basal.vs.ml$P.Value)*sign(basal.vs.ml$logFC)
names(rank)=basal.vs.ml$ENTREZID
...
library(fgsea)
fgseaRes = fgseaSimple(pathways = Mm.c2,stats=rank,nperm=1000,maxSize = 200,minSize = 15)
...
pres = data.frame(result) %>% filter(ES>=0)%>% arrange(padj,desc(ES),pathway)
nres = data.frame(result) %>% filter(ES<0)%>% arrange(padj,ES,pathway)
...
mat = cbind(rank,rank)
colnames(mat)[1]="Basal vs ML"
colnames(mat)[2]="Basal vs ML"

mresp = GOenrichment::runGORESP(fdrThresh = 0.05,mat=mat,
    coln=1,curr_exp = colnames(mat)[1], sig = rank[1500], 
    bp_input = Mm.c2,go_input = NULL,minSetSize = 15, 
    maxSetSize = 200)
intersect(fposgsea$pathway[1:20],mresp$enrichInfo$term[1:20])
```
...
#####
vis = visSetup(enrichInfo = mresp$enrichInfo,edgeMat = mresp$edgeMat)
############
...
```{r}
vis = visSetup(mresp$enrichInfo,mresp$edgeMat)
...
```
Use the mresp enrichInfo and plug it into this revised function:
```{r newmresp}


if (file.exists("newmresp.rds")) {
  newmresp <- readRDS("newmresp.rds")
} else {
  
newmresp = calcOverGOcoeff(select_fun=overlapcoeff,cutoff=0.7,res = mresp$enrichInfo,padj="FDR",term="term",goont=Mm.c2)
  saveRDS(newmresp, "newmresp.rds")
}



w=which(mresp$edgeMat$source%in% newmresp$id)
ww=which(mresp$edgeMat$target[w]%in% newmresp$id)
newedge = mresp$edgeMat[w[ww],]

v = visSetup(newmresp,newedge)
