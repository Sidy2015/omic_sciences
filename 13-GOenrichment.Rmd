...
found
[here](https://github.com/gurinina/omic_sciences/blob/main/13-GSEA.Rmd).
...
calcOverGOcoeff= function(select_fun = overlapcoeff,res, cutoff = 0.6, showCategory = 200, keytype = "GENENAME",ont = "BP",orgDb = "org.Sc.sgd.db") { 
    ...
    colnames(mx) = res$ID[1:length(showCategory)]
    rownames(mx) = res$ID[1:length(showCategory)]
...
big for visualization, but we've perserved the top annotations:
...
hensembl = useDataset("hsapiens_gene_ensembl",mart=ensembl)
...
So the take hom lesson is that in general we don't see 
...
>>>>>>> 4ee818e (GO enrichment)
>>>>>>> 43efd3b (GO enrichment)
>>>>>>> debdc97 (GO enrichment)
